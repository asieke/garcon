...
Everything is driven by `install.sh` in the repository root. Read `README.md` first for
what the proxy does and how harnesses are pointed at it.
...
4. **Point harnesses at it** using the environment variables and Codex provider settings
   ...
   `/api/usage`.
5. **Remove**: `./install.sh --uninstall`. The usage log at
